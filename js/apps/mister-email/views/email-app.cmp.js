...
export default {
        ...
        <button class="btn-menu-mail" @click="toggleMenu">☰</button>
                ...
                <div class="main-container">
                    ...
                    <div class="sec-container" >
                        ...
                        <email-compose v-if="composeMode" :initialValues="composeInitialValues" @mailSent="addMail" @backToMails="backToMails"></email-compose>
  ...
  created() {
    ...
    if (queryParams.subject && queryParams.body) {
  ...
  computed: {
    mailsToShow() {
      ...
      if (!this.filterBy)
        return filteredMails.sort((c1, c2) => c2.sentAt - c1.sentAt);

      if (!this.sortBy) {
        filteredMails = this.mails.filter(
          (mail) =>
        ...
        filteredMails = this.mails.filter(
          (mail) => regex.test(mail.subject) && mail.isStarred
        );
        ...
        filteredMails = this.mails.filter(
          (mail) => regex.test(mail.subject) && mail.isTrash
        );
      } else if (this.sortBy === "sent") {
        filteredMails = this.mails.filter(
          (mail) => regex.test(mail.subject) && mail.isSent
        );
      } else if (this.sortBy === "draft") {
        filteredMails = this.mails.filter(
          (mail) => regex.test(mail.subject) && mail.isDraft
        );
      }

      if (this.setMailsBySubject && this.setMailsByLatest)
        return filteredMails.sort((c1, c2) => {
      ...
      if (this.setMailsByLatest)
        return filteredMails.sort((c1, c2) => c2.sentAt - c1.sentAt);
      else if (this.setMailsBySubject)
        return filteredMails.sort((c1, c2) => {
    ...
    calcMailsByFolders() {
      return {
        readMails: this.mails.filter(
          (mail) =>
            mail.isRead && !mail.isSent && !mail.isTrash && !mail.isDraft
        ).length,
        unreadMails: this.mails.filter(
          (mail) =>
            !mail.isRead && !mail.isSent && !mail.isTrash && !mail.isDraft
        ).length,
