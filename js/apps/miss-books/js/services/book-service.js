...
const API_BOOKS_KEY = 'BooksFromGoogle';
_createBooks();
...
function getApiBooks(bookName) {
  const bookSearch = utilService.loadFromStorage(API_BOOKS_KEY) || {};
  if (bookSearch[bookName]) return Promise.resolve(bookSearch[bookName])

  return axios.get(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${bookName}`)
    .then(res => {
      res.data;
      bookSearch[bookName] = res.data.items;
      utilService.saveToStorage(API_BOOKS_KEY, bookSearch);
